...
int main(){
    ...
    BmodeClass *currentScanline = scanlineHead; // currentScanline pointer keeps track of w hich object is the current scanline
    dataBuffer *dataIteration = dataHead;

    // iterative creation of a BmodeClass linked list
    for (int i = 1; i < parameters->getNumScanline(); i++)
    {
        dataIteration = dataIteration->next;
        BmodeClass *newNode = new BmodeClass(parameters, dataIteration, i);
        currentScanline->next = newNode;
        currentScanline = currentScanline->next; // advance pointer to next entry
        currentScanline->next = NULL; // assign pointer as NULL
    }
    
    /*
    file compilation text:
